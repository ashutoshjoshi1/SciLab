...
class MeasurementRunner:
    def __init__(self, cfg: AppConfig):
        self.cfg = cfg
        self.spec = AvantesController(dll_path=self.cfg.avantes.dll_path)
    ...
    def _disconnect_devices(self):
        ...
        for i in (self.obis, self.cube, self.relay):
            try:
                if i:
                    i.close()
            except: pass
    ...
    def _laser_on(self, ls: LaserSpec):
        if ls.type == "CUBE":
            ...
            if not self.obis or ls.channel is None: raise RuntimeError("OBIS port not connected(measurement.py)")
            self.obis.on(ls.channel)
            if ls.power_w is not None:
                self.obis.set_power_w(ls.channel, ls.power_w)
        elif ls.type == "RELAY":
            if not self.relay or ls.relay_channel is None: raise RuntimeError("Relay port error(measurement.py)")
    ...
    def run(self, on_live: Optional[Callable[[np.ndarray, float,float,str], None]] = None) -> MeasurementResult:
        ...
        try:
            ...
            auto = AutoIT(AutoITParams(
                it_min_ms= p.it_min_ms,
                it_max_ms= p.it_max_ms,
                target_low=p.target_low,
                target_high=p.target_high,
                step_up_ms=p.step_up_ms,
                step_down_ms=p.step_down_ms,
                max_adjust_iters=p.max_adjust_iters,
            ...
            for ls in self.cfg.lasers:
                ...
                y_dark = self.spec.read_many(self.cfg.measure.n_sig)
